...
def get_stats() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        url = f"{{BASE_URL}}/v1/reports/stats/{{fecha_inicial}}/{{fecha_final}}"
        headers = {{"token": config["token"]}}
        logger.info(
            "Obteniendo estadísticas {report_code} para período: %s - %s",
            fecha_inicial,
            fecha_final,
        )
...
def generate_report() -> bool:
    ...
    try:
        ...
        logger.info(
            "Datos obtenidos - Stats: %s, {reportes_label}: %s, Usuario: %s",
            "✓" if stats else "✗",
            "✓" if versus else "✗",
