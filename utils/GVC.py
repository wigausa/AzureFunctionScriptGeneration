...
TOKEN_USERS = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjoiYWRtaW53aWdhIiwiaWQiOjI3MCwiZXhwaXJlZERhdGUiOiIyMDE5LTA3LTIyVDExOjAwOjMyLjA2NzU2ODYtMDU6MDAifQ.BXMx2BKIbkJyD_jRrfhY6Sj_SJbo8gWM8wHghzFvrT0"
BASE_URL = "https://superwicloudapi.azurewebsites.net/api/v1"
AZURE_FUNCTION_URL = os.getenv("AZURE_FUNCTION_GVC_URL", "http://localhost:7071/api/GVC")
...
def get_data_informe() -> Optional[Dict[str, Any]]:
    ...
    if result and 'configuration' in result:
        logger.info("Configuración del informe obtenida exitosamente")
        return result
    
    logger.error("No se pudo obtener configuración del informe")
    return None

def get_reportes_graficos() -> Optional[Dict[str, Any]]:
    """Obtiene los datos de gráficos versus desde la API."""
    informe = get_data_informe()
    if not informe:
        logger.error("No se pudo obtener configuración del informe")
        return None
    
    url = f"{{BASE_URL}}/reports/multipleTableReport"
...
def get_stats() -> Optional[Dict[str, Any]]:
    ...
    fecha_inicial = informe['configuration']['fechas']['fechaInicial']
    fecha_final = informe['configuration']['fechas']['fechaFinal']
    url = f"{{BASE_URL}}/reports/stats/{{fecha_inicial}}/{{fecha_final}}"
    headers = {{'token': informe['configuration']['token']}}
    
    result = make_request('POST', url, headers=headers, json_body=informe['configuration']['sensores'])
    if result:
...
def send_to_azure_function(stats: Any, versus: Any, datos_usuario: Any) -> Optional[bytes]:
    """Envía los datos a la Azure Function y retorna el contenido de la respuesta."""
    payload = {{
        "stats": stats,
        "versus": versus,
    ...
    try:
        logger.info("Enviando datos a Azure Function GVC...")
        response = requests.post(AZURE_FUNCTION_URL, json=payload, timeout=60)
        response.raise_for_status()
        logger.info("Datos enviados correctamente a Azure Function GVC")
        return response.content
    except requests.RequestException as e:
        logger.error(f"Error al enviar datos a Azure Function GVC: {{e}}")
        return None

def generate_report() -> bool:
    """Función principal para generar el reporte."""
    logger.info("=== Iniciando generación de reporte GVC ===")
    ...
    try:
        ...
        if not all([stats is not None, versus is not None, datos_usuario is not None]):
            logger.error("No se pudieron obtener todos los datos necesarios")
            return False
        ...
        if pdf_data:
            logger.info("=== Reporte GVC generado exitosamente ===")
            return True
        else:
            logger.error("Error al generar el reporte GVC")
            return False
            
    except Exception as e:
        logger.error(f"Error inesperado durante la generación del reporte: {{e}}")
        return False
