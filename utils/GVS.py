#!/usr/bin/env python3
"""
Generador de scripts GVS (mensuales) para Azure WebJobs
...
def create_script(report_id: str, output_dir: str = "../GraficaVersus") -> str:
    """Crea un script GVS personalizado para el report_id especificado."""
    try:
        ...
        if not informe_data or 'id' not in informe_data:
            ...
            file_name = f"GVS_{report_id}.py"
        else:
            file_name = f"GVS_{informe_data['id']}.py"
...
if __name__ == "__main__":
    ...
    logger.info(f"🚀 Iniciando generación de script GVS para report_id: {report_id}")
